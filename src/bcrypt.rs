...
impl fmt::Display for BcryptError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            BcryptError::InvalidCost(ref len) => write!(f, "Invalid cost length: {}, must be less than 32", len),
...
impl error::Error for BcryptError {
    fn description(&self) -> &str {
        match *self {
            BcryptError::InvalidCost(_) => "the bcrypt cost provided was invalid",
            BcryptError::EmptyPassword() => "the password to hash was invalid",
        }
    }
...
fn setup(cost: u32, salt: &[u8; 16], key: &[u8]) -> Result<Blowfish, BcryptError> {
    if cost >= 32 {
...
pub fn bcrypt(cost: u32, salt: &[u8; 16], password: &[u8]) -> Result<[u8; 24], BcryptError> {
    if password.len() < 1 {
        return Err(BcryptError::EmptyPassword());
    }
    let mut output = [0u8; 24];

    let state = try!(setup(cost, salt, password));
...
mod test {
    ...
    fn test_openwall_test_vectors() {
        ...
        for test in tests.iter() {
            let result = bcrypt(test.cost, &test.salt, &test.input[..]).unwrap();
            assert_eq!(result[0..23], test.output[0..23]);
        }
    }
}
...
mod bench {
    ...
    pub fn bcrypt_16_5(bh: & mut Bencher) {
        ...
        let salt = [0u8; 16];
        bh.iter( || {
            let result = bcrypt(5, &salt, &pass).unwrap();
        });
    }
}
